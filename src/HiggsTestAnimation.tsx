...
const SCRIPT = `This is a test of the higgs-audio foundational TTS component. 
The speech will generate automatically using the local open-source model. 
You can easily integrate this into any Remotion animation by importing the useHiggsAudio hook.`;

export const HiggsTestAnimation: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps, durationInFrames, width, height } = useVideoConfig();

  // Use the foundational higgs-audio hook
  const {
    narrationAudioPath,
    isGenerating,
    speechDuration,
  ...
  // Background gradient animation (different color scheme for higgs-audio)
  ...
  return (
    ...
    >
      ...
      {/* Narration Audio from higgs-audio */}
      {narrationAudioPath && (
        <Audio
      ...
      {!isServerAvailable && !isGenerating && (
        <div
          style={{
            position: 'absolute',
            top: 40,
        ...
        >
          ⚠️ Higgs Audio server not available. Please start the local server.
        </div>
      )}

      {/* Manual Generate Button (if auto-generate fails) */}
      {!narrationAudioPath && !isGenerating && !error && isServerAvailable && (
        <div
          style={{
            position: 'absolute',
            top: 40,
      ...
      {isGenerating && (
        ...
        >
          🔄 Generating speech with higgs-audio...
        </div>
      )}
      ...
      >
        Higgs Audio Foundation Component Test
      </div>
      ...
      >
        ...
        >
          <div style={{ marginBottom: 15, fontWeight: 'bold', fontSize: 20 }}>
            ✓ Higgs Audio Component Working
          </div>
          <div style={{ opacity: 0.9 }}>
            Server Status: {isServerAvailable ? '✅ Available' : '❌ Not Available'}
          </div>
          ...
          <div style={{ opacity: 0.7, marginTop: 15, fontSize: 14 }}>
            Using open-source higgs-audio local model
          </div>
        </div>
      </div>
    </AbsoluteFill>
